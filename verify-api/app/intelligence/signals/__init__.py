...
Runs 9 signal libraries against bank transactions and maps
...
Interface notes (actual signatures discovered from source):
  ...
  - p2p_transfer.run(transactions)                                 — NO loader param
"""
import re
from datetime import datetime, timezone

from ..las_score import calculate_las
from ..verifyplus import build_verifyplus_result as _build_vp
...
# 17-slot mapping — frontend renders dynamically from result.signals array
SIGNAL_SLOTS = [
    ...
    {"name": "P2P / Mobile Transfers",        "cat": "FLOW · OBFUSCATION",      "types": ["p2p_transfer"],       "keywords": []},
]
...
INTEL_TEMPLATES = {
    ...
    "P2P / Mobile Transfers": {
        ...
        "rec": "Medium Recovery",
        "rec_cls": "medium",
        "tier": "Mobile Payment Platforms",
        "path_template": (
            "Subpoena {operators} for recipient KYC records, linked bank accounts, "
            "and complete transaction history. Recurring payments to named individuals "
            "from a business account may indicate undisclosed payroll, related-party "
            "transfers, or personal asset concealment."
        ),
    },
}
...
def run_signals(transactions: list, loader=None) -> list:
    ...
    # behavioural, liability, and p2p_transfer accept only transactions
    for module in [behavioural, liability, p2p_transfer]:
        try:
...
def build_verify_result(
    ...
    try:
        verify_plus = _build_vp(matter_id, raw_signals, transactions, jurisdiction)
    except Exception:
